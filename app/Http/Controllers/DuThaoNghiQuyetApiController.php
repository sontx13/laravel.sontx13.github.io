...
class DuThaoNghiQuyetApiController extends Controller
{

    public $successStatus = Response::HTTP_OK;

    public function GetDsDuThaoNghiQuyet(Request $request)
    {
        try {
            $kyhopid = $request->kyhopid;
            $folderid = $request->folderid;
            $tenantid = Auth::user()->tenant_id;
            $tailieu = $this->getTaiLieuKyhops($tenantid, $kyhopid, $folderid, LoaiTaiLieuEnum::DuThaoNghiQuyet);
    ...
    public function SearchDuThaoNghiQuyet(Request $request)
    {
        try {
            $term = $request->term;
            $tenantid = Auth::user()->tenant_id;
            $kyhopid = $request->kyhopid;
            $tailieu = $this->fulltextSearchTaiLieu($tenantid, LoaiTaiLieuEnum::DuThaoNghiQuyet, $kyhopid, $term);
            return response()->json([
                'error_code' => 0,
                'data' =>  $tailieu,
            ], Response::HTTP_OK);
